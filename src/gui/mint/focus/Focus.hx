package gui.mint.focus;

import gui.mint.types.Types;
import gui.mint.types.Types.Helper.in_rect;

class Focus {

    public var canvas: gui.mint.Canvas;

    public function new(_canvas:gui.mint.Canvas) {
    ...
    function keydown(e:KeyEvent, target:gui.mint.Control) {
        if(canvas.focused != null && canvas.focused.key_input) {
            canvas.focused.keydown(e);
        }
    }

    function keyup(e:KeyEvent, target:gui.mint.Control) {
        if(canvas.focused != null && canvas.focused.key_input) {
            canvas.focused.keyup(e);
        }
    }

    function textinput(e:TextEvent, target:gui.mint.Control) {
        if(canvas.focused != null && canvas.focused.key_input) {
            canvas.focused.textinput(e);
        }
    }

    function mousedown(e:MouseEvent, target:gui.mint.Control) {
    ...
    function mouseup(e:MouseEvent, target:gui.mint.Control) {
        if(canvas.focused != null && canvas.focused.mouse_input) {
    ...
    function mousewheel(e:MouseEvent, target:gui.mint.Control) {
        if(canvas.focused != null && canvas.focused.mouse_input) {
    ...
    function get_markable(_target:gui.mint.Control, _x:Float, _y:Float) {
