...
class Macros {
    ...
    macro public static function assert(expr:Expr, ?reason:ExprOf<String>) {
            ...
            return macro @:pos(Context.currentPos()) {
                if(!$expr) throw gui.mint.core.Macros.DebugError.assertion( '$str' + $reason);
            }
        #end
        return macro null;
    } //assert
    ...
    macro public static function assertnull(value:Expr, ?reason:ExprOf<String>) {
            ...
            return macro @:pos(Context.currentPos()) {
                if($value == null) throw gui.mint.core.Macros.DebugError.null_assertion('$str was null' + $reason);
            }
        #end
        return macro null;
    } //assertnull
